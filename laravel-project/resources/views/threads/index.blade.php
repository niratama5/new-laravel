            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    @foreach ($threads as $thread)
                      ...
                      <div>
                        <a href="{{ route('threads.show-reply', $thread->thread_id) }}" class="text-blue-500">返信</a>  
                      </div>
                      <button id="show_replies_{{$thread->thread_id}}" data-thread-id="{{$thread->thread_id}}">返信を表示</button>
                      <div id="replies_container_{{$thread->thread_id}}"></div>
                    @endforeach
                    @if ($threads->isEmpty())
